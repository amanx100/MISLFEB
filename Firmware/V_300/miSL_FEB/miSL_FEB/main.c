...
#define printerPrintDisabled (PINB & (1<<PB1))

#define iMarkSensorHoldStatus (PINB & (1<<PB5))
#define iMarkSensorHold_ON (PORTB |= (1<<PB5))
#define iMarkSensorHold_OFF (PORTB &= ~(1<<PB5))

#define printCommandOutput_Status (PIND & (1<<PD5))
#define printCommandOutput_ON (PORTD |= (1<<PD5))
#define printCommandOutput_OFF (PORTD &= ~(1<<PD5))

#define printSuceessOutput_Status (PINC & (1<<PC0))
#define printSuccessOutput_ON (PORTC |= (1<<PC0))
#define printSuccessOutput_OFF (PORTC &= ~(1<<PC0))

#define printerReadyOutput_Status (PINC & (1<<PC1))
#define printerReadyOutput_ON (PORTC |= (1<<PC1))
#define printerReadyOutput_OFF (PORTC &= ~(1<<PC1))

#define visualPulseOutput_TOGGLE (PORTD ^= (1<<PD4))
